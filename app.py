...
def update_movie_likes_by_id(movie_id, username, action_type):
    ...
    if action_type == 'like':
        ...
        # Check if recent liked genres overlap with the genre of the disliked movie
        if genre in (user.recent_liked_genre or '').split(','):
            user.recent_liked_genre = ''  # Clear recent liked genres if there is overlap
